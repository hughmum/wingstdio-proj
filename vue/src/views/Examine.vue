<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="headerBg" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="borrowPerson" label="借用人" width="120"></el-table-column>
      <el-table-column prop="borrowReason" label="借用理由"></el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-popconfirm
              ...
              icon-color="red"
              title="确定批准吗？"
              @confirm="handleAgree(scope.row)"
          >
          <el-button type="success" slot="reference">批准同意<i class="el-icon-edit"></i></el-button>
          </el-popconfirm>
          <el-button type="danger" slot="reference" @click="handleRefuse(scope.row)">拒绝申请<i class="el-icon-remove-outline"></i></el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <el-dialog title="拒绝" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="60px" size="small">
        <el-form-item label="拒绝原因">
          <el-input type="textarea" v-model="form.refuseReason" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="refuseRequest">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods:{
    ...
    handleAgree(row) {
      ...
      this.form.currentExamineId = this.user.id
      console.log(this.form)
      this.request.post("/examine/agree",this.form).then(res=>{
        if(res.code==='200'){
    ...
    refuseRequest() {
      this.form.currentExamineId = this.user.id
      console.log(this.form)
      this.request.post("/examine/refuse",this.form).then(res=>{
        if(res.code==='200'){
